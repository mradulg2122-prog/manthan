...
Sends premium confirmation emails with QR code attachments via SMTP.
"""
...
def _send_email(msg: MIMEMultipart) -> None:
    ...
    if not password:
        raise ValueError("SMTP_PASSWORD (Google 16-character App Password) must be set in backend/.env")
...
def send_qr_email(
    ...
    """Send a registration confirmation email with QR code attached and embedded via SMTP."""
...
<body>
  <div class="container">
    ...
    # Attach alternative (plain text + html)
...
def send_certificate_email(
    ...
    msg["From"] = f"MANTHAN — Saturangle Debate Club <{sender_email}>"
    msg["To"] = recipient_email
    msg["Subject"] = f"{event_name} — Your Certificate of Participation"

    body = (
        f"Hello {recipient_name},\n\n"
        f"Thank you for participating in {event_name} (PRARAMBH 2K26).\n\n"
        f"Please find your official certificate attached.\n\n"
        f"Your Registration ID: {registration_id}\n\n"
        f"Regards,\n"
        f"Saturangle Debate Club & EventFlow Team"
    )
